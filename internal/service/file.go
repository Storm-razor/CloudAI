...
// @brief 上传文件
// ---------------------------
func (fs *fileService) UploadFile(userID uint,
	...
	key := GenerateStorageKey(userID, fileID)
...
func (fs *fileService) DownloadFile(fileID string) (*model.File, []byte, error) {
	...
	log.Printf("请求下载文件:%s", fileMeta.StorageKey)
...
// ---------------------------
// @brief GetFileIDPath 生成基于文件ID的路径
// ---------------------------
