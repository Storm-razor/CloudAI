...
func (fc *FileController) Upload(ctx *gin.Context) {
	...
	if err != nil {
		response.ParamError(ctx, errcode.ParamBindError, "解析表单失败")
		return
	}

	file, err := fileHeader.Open()
	if err != nil {
		response.ParamError(ctx, errcode.FileParseFailed, "解析文件失败")
		return
	}
	defer file.Close()
...
func (fc *FileController) BatchMove(ctx *gin.Context) {
	var req ginmodel.BatchMoveRequest
...
func (fc *FileController) Rename(ctx *gin.Context) {
	var req ginmodel.RenameRequest
	if err := ctx.ShouldBindJSON(&req); err != nil {
		response.ParamError(ctx, errcode.ParamBindError, "参数错误")
		return
	}
